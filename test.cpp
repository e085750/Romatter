...
bool MyApp::OnInit()
{
  MyFrame *frame = new MyFrame( "Hello World", wxPoint(500, 50), wxSize(340, 450) );
  frame->Show( true );
  return true;
}
...
void MyFrame::OnStartSimplePopup(wxCommandEvent& event)
{
    ...
    //printf("%s\n", "あ");
    printf("%s\n", (const char *)box_text.c_str());
    //printf("%s\n", box_text.wx_str());
    //printf("%s\n", box_text.mb_str());
    //printf("%s\n", box_text.ToStdString());
    //printf("%s\n", box_text.ToStdString().c_str());
    printf("%s\n", (const char *)box_text.utf8_str());

    this->twit.post((const char *)box_text.utf8_str());
    //this->twit.post("てすと");
